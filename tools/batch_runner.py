...
def main():
    ...
    if os.path.exists(output_path):
        del_choice = input(f"\n⚠️ 發現已存在檔案 {default_file}，是否要先刪除以確保筆數精準？(y/n): ").strip().lower()
        if del_choice == 'y':
    ...
    for i, seq in enumerate(scenarios):
        ...
        cmd = [sys.executable, auto_dataset_path, "--output", output_path, "--mode", mode, "--count", count]
        if sequence:
    ...
    if not os.path.exists(output_path):
        ...
        formatted_output_path = os.path.join(dir_name, f"train_{file_root}{file_ext}")
        shuffled_output_path = os.path.join(dir_name, f"train_{file_root}_shuffled{file_ext}")
        ...
        if process_dataset(output_path, formatted_output_path):
            ...
            if shuffle_dataset(formatted_output_path, shuffled_output_path):
                ...
                print(f"  [2] 訓練格式: {formatted_output_path}")
